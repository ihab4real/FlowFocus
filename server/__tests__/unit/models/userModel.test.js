...
describe('User Model - Unit Tests', () => {
  ...
  describe('Schema Validation', () => {
    ...
    it('should fail if email is missing', async () => {
        ...
        const user = new User(userData);
        await expect(user.validate()).rejects.toThrow('Please provide your email');
    });
    ...
    it('should fail if password is too short', async () => {
      const userData = createValidUserData({ password: 'short', passwordConfirm: 'short' });
      const user = new User(userData);
    ...
    it('should fail if passwordConfirm is missing', async () => {
      ...
      const user = new User(userData);
      await expect(user.validate()).rejects.toThrow('Please confirm your password');
    });

    it('should fail if passwords do not match', async () => {
      const userData = createValidUserData({ passwordConfirm: 'differentPassword' });
      const user = new User(userData);
  ...
  describe('Helper Functions (Simulating Hook Logic)', () => {
    ...
    describe('_hashPassword', () => {
      it('should hash password and unset passwordConfirm if password is modified', async () => {
        ...
        expect(await bcrypt.compare('plainPassword', userInstance.password)).toBe(true);
  ...
  describe('Instance Methods', () => {
    ...
    beforeEach(async () => {
      ...
      userInstance = createValidUserData({ password: plainPassword, passwordConfirm: plainPassword });
      ...
      userInstance.correctPassword = User.schema.methods.correctPassword.bind(userInstance);
      userInstance.changedPasswordAfter = User.schema.methods.changedPasswordAfter.bind(userInstance);
      userInstance.createPasswordResetToken = User.schema.methods.createPasswordResetToken.bind(userInstance);
    });

    describe('correctPassword', () => {
      it('should return true for correct password', async () => {
        // Pass the plain password and the *now hashed* password from userInstance
        await expect(userInstance.correctPassword(plainPassword, userInstance.password)).resolves.toBe(true);
      });

      it('should return false for incorrect password', async () => {
        await expect(userInstance.correctPassword('wrongPassword', userInstance.password)).resolves.toBe(false);
      });
    });
