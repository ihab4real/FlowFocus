...
const ModeSelector = ({ isFullscreen }) => {
  ...
  const handleModeChange = (newMode) => {
    ...
    if (newMode === TIMER_MODES.FOCUS) {
      setTimeLeft(
        (settings?.focusDuration || DEFAULT_SETTINGS.FOCUS_DURATION) * 60
      );
    } else if (newMode === TIMER_MODES.SHORT_BREAK) {
      setTimeLeft(
        (settings?.shortBreakDuration ||
          DEFAULT_SETTINGS.SHORT_BREAK_DURATION) * 60
      );
    } else {
      setTimeLeft(
        (settings?.longBreakDuration || DEFAULT_SETTINGS.LONG_BREAK_DURATION) *
          60
      );
    }
    setIsActive(false);
  ...
  return (
    <div className={`flex gap-3 ${isFullscreen ? "mb-6" : "mb-4"}`}>
      ...
      <Button
        ...
        className={
          mode === TIMER_MODES.SHORT_BREAK ? BUTTON_CLASSES.SHORT_BREAK : ""
        }
        size={isFullscreen ? "default" : "sm"}
      >
        Short Break
      ...
      <Button
        ...
        className={
          mode === TIMER_MODES.LONG_BREAK ? BUTTON_CLASSES.LONG_BREAK : ""
        }
        size={isFullscreen ? "default" : "sm"}
      >
        Long Break
